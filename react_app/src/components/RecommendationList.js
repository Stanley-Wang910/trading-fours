...
function RecommendationsList({ recommendations, onRecommendations, setIsLoading, query, position, onTogglePosition, setFavoritedTracks, animateOut, setAnimateOut}) {
  //State variables
    ...
    <div className="relative">
      <div className={`mx-auto p-4 transition-transform duration-500 ${position === "left" ? "-translate-x-full" : ""}`}>
        ...
        {/* <div className="recommendations-container p-4 bg-gradient-to-r from-gray-800 to-gray-600 shadow-2xl rounded-2xl transform overflow"> */}
          <div className={`recommendations-container p-5 w-full max-w-screen-sm bg-gradient-to-tr from-gray-900 via-gray-800 to-blue-900 shadow-2xl rounded-2xl relative overflow-hidden container-transition opacity-0 ${animate ? 'recs-fade-up' : ''} ${animateOut ? 'recs-fade-out' : ''}`} style={{ '--container-height': `${containerHeight}px` }}>
          {showMeteors && (
            <Meteors
              number={25}
